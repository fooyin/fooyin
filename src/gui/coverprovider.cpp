...
namespace
{
    ...
    {
        return Fooyin::Gui::coverPath() + key + u".jpg"
        _s;
    }
    ...
    QImage readImage(const QString& path, int requestedSize, const QString& hintType)
    {
        ...
        if (!reader.canRead())
        {
            qCDebug(COV_PROV) << "Failed to use format hint" << formatHint << "when trying to load" << hintType <<
                "cover";
...
namespace Fooyin
{
    class FYGUI_NO_EXPORT CoverProvider::CoverProviderPrivate
    {
        public
        :
        explicit CoverProviderPrivate (CoverProvider
        *self, std::shared_ptr<AudioLoader>
        audioLoader,
    ...
    CoverProvider::CoverProviderPrivate::CoverProviderPrivate(CoverProvider* self,
    ...
    void CoverProvider::CoverProviderPrivate::processCoverResult(const CoverLoader& loader)
    {
        ...
        emit
        m_self->coverAdded(loader.track);
    }
