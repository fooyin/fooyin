...
void DspModel::setCheckable(bool checkable)
{
    if(std::exchange(m_checkable, checkable) != checkable) {
        beginResetModel();
        endResetModel();
    }
}
...
QVariant DspModel::data(const QModelIndex& index, int role) const
{
    ...
    switch(role) {
        ...
        case(Qt::CheckStateRole):
            return m_checkable ? QVariant{dsp.enabled ? Qt::Checked : Qt::Unchecked} : QVariant{};
        case(Role::Id):
            return dsp.id;
...
bool DspModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    if(!checkIndex(index, CheckIndexOption::IndexIsValid)) {
        return false;
    }
    ...
    auto& dsp = m_dsps[static_cast<size_t>(index.row())];
    bool changed{false};

    if(role == Qt::CheckStateRole && m_checkable) {
        const bool enabled = (value.toInt() == Qt::Checked);
        changed            = (std::exchange(dsp.enabled, enabled) != enabled);
    }
    else if(role == Role::Enabled) {
