...
- (MPRemoteCommandHandlerStatus)seekForwardCommand {
    self.plugin->playerController()->seekForward(self.plugin->settings()->value<Fooyin::Settings::Gui::SeekStep>());
    return MPRemoteCommandHandlerStatusSuccess;
}
- (MPRemoteCommandHandlerStatus)seekBackwardCommand {
    self.plugin->playerController()->seekBackward(self.plugin->settings()->value<Fooyin::Settings::Gui::SeekStep>());
    return MPRemoteCommandHandlerStatusSuccess;
}
