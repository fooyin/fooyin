...
uint64_t nominalFrameDurationNs(int sampleRate)
{
...
uint64_t mappedFrameDurationNs(const Fooyin::ProcessingBuffer& buffer, int inputRate, int outputRate)
{
    const uint64_t inputSourceNs
        = buffer.sourceFrameDurationNs() > 0 ? buffer.sourceFrameDurationNs() : nominalFrameDurationNs(inputRate);

    const double mappedNs = (static_cast<double>(inputSourceNs) * static_cast<double>(std::max(1, inputRate)))
...
void ResamplerDsp::flush(ProcessingBufferList& chunks, FlushMode mode)
{
    ...
    const uint64_t sourceFrameDurationNs = m_outputSourceFrameDurationNs > 0
                                             ...
                                             : nominalFrameDurationNs(m_outputFormat.sampleRate());
    for(int drainIter = 0; drainIter < MaxDrainIterations; ++drainIter) {
        ...
        outBuffer->setSourceFrameDurationNs(sourceFrameDurationNs);
        advanceOutputCursor(outFrames, sourceFrameDurationNs);
    }
...
void ResamplerDsp::processBuffer(const ProcessingBuffer& buffer, ProcessingBufferList& output)
{
    ...
    const uint64_t sourceFrameDurationNs
        = mappedFrameDurationNs(buffer, m_inputFormat.sampleRate(), m_outputFormat.sampleRate());
...
bool ResamplerDsp::shouldResampleInputRate(int inputRate) const
{
...
void ResamplerDsp::advanceOutputCursor(int outFrames, uint64_t sourceFrameDurationNs)
{
    if(outFrames <= 0 || sourceFrameDurationNs == 0) {
        return;
    }

    const double advanceNsFull
        = (static_cast<double>(outFrames) * static_cast<double>(sourceFrameDurationNs)) + m_outputCursorRemainderNs;
