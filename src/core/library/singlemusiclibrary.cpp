...
void SingleMusicLibrary::refreshTracks(const TrackList& result)
{
    //    m_trackStore.clear();

    m_trackStore->add(result);
    //    m_trackStore.sort(m_order);

    emit tracksLoaded(m_trackStore->tracks());
}
...
int SingleMusicLibrary::trackCount() const
{
