...
template <typename T>
T quantiseToSigned(double sample, double scale, T minValue, T maxValue)
{
    const double low  = static_cast<double>(minValue) / scale;
    const double high = static_cast<double>(maxValue) / scale;

    const double x    = std::clamp(sample, low, high);
...
void writeSample(std::byte* dst, SampleFormat format, double normalized)
{
    switch(format) {
        case SampleFormat::U8: {
            const auto centered = quantiseToSigned<int16_t>(normalized, 128.0, int16_t{-128}, int16_t{127});
            const auto value    = static_cast<uint8_t>(centered + 128);
            std::memcpy(dst, &value, sizeof(value));
            break;
        }
        case SampleFormat::S16: {
            const auto value = quantiseToSigned<int16_t>(normalized, 32768.0, std::numeric_limits<int16_t>::min(),
                                                         std::numeric_limits<int16_t>::max());
            std::memcpy(dst, &value, sizeof(value));
            break;
        }
        case SampleFormat::S24In32: {
            const auto value = quantiseToSigned<int32_t>(normalized, 8388608.0, int32_t{-8388608}, int32_t{8388607});
            std::memcpy(dst, &value, sizeof(value));
            break;
        }
        case SampleFormat::S32: {
            const auto value = quantiseToSigned<int32_t>(normalized, 2147483648.0, std::numeric_limits<int32_t>::min(),
...
AudioBuffer makeBuffer(SampleFormat format, int channels = Channels, int frames = Frames)
{
    ...
    for(int frame{0}; frame < frames; ++frame) {
        for(int channel{0}; channel < channels; ++channel) {
            ...
            const auto offset   = (frame * channels + channel) * bps;
